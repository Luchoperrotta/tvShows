class SeasonsController < ApplicationController
  ...
  def index
   ...
   #esto es para la filtracion en el index de series cuando quiero ver sus temporadas

   @seasons = @seasons.joins(:espisode).where(:espisode => {:id => params[:espisode_id]}) if params[:espisode_id]
   @seasons = @seasons.where(["number LIKE :filte", :filte => ("%" + params[:filte] + "%")]) unless params[:filte].blank?
  #esto es para la filtracion en el index de episodios cuando quiero ver sus temporadas
